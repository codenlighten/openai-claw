...
export function App({ agent, config, permissions, hooks }) {
    ...
    useEffect(() => {
        permissions.setPrompter((req) => new Promise((resolve) => {
            setPending({ ...req, resolve: (answer) => { setPending(null); resolve(answer); } });
        }));
    }, [permissions]);
    useInput((inputChar, key) => {
        if (pending)
            return; // PermissionPrompt owns input while open
            ...
            switch (evt.type) {
                ...
                case "text":
                    if (streamBuffer) {
                        ...
                        push({ kind: "assistant", id: nextId(), text: evt.data });
                    }
                    break;
                case "tool_call": {
                    if (streamBuffer) {
                ...
                case "usage": {
                    ...
                    if (typeof u.totalCostUSD === "number")
                        setTotalCostUSD(u.totalCostUSD);
                    break;
                }
                case "compaction": {
                    const d = evt.data;
    ...
    return (React.createElement(Box, { flexDirection: "column" },
        React.createElement(Static, { items: history }, (item) => React.createElement(MessageView, { key: item.id, item: item })),
        liveTool && React.createElement(MessageView, { item: liveTool }),
        streamingItem && React.createElement(MessageView, { item: streamingItem }),
        pending && React.createElement(PermissionPrompt, { pending: pending }),
        React.createElement(SlashSuggest, { input: input, config: config }),
        React.createElement(StatusBar, { config: config, busy: busy, totalTokens: totalTokens, totalCostUSD: totalCostUSD }),
        React.createElement(Box, { marginTop: 1 },
            React.createElement(Text, { color: "cyan", bold: true }, "❯ "),
            React.createElement(TextInput, { value: input, onChange: setInput, onSubmit: submit, placeholder: busy ? "(thinking… input queues)" : "ask anything or /help" }))));
}
