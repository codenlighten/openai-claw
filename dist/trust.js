...
export function isProjectTrusted(config) {
...
export function markProjectTrusted(config) {
    const s = readUserSettings(config);
    const list = Array.isArray(s.trustedProjects) ? s.trustedProjects.slice() : [];
    const here = path.resolve(config.workdir);
    if (!list.some((p) => path.resolve(p) === here))
        list.push(here);
 ...
 */
export async function resolveProjectTrust(config, opts = { interactive: true }) {
