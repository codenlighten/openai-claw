import type { AnchorProof, AnchorCalendarResponse } from "./anchor.js";
...
export function buildOtsFile(digestHex: string, calendarResponseBase64: string): Buffer | null {
...
export interface OtsExport {
  /** e.g. "alice", "bob", "finney" — short suffix used in the output filename. */
  shortName: string;
  url: string;
  /** Bytes of the resulting .ots file, ready to write or hand to `ots verify`. */
  bytes: Buffer;
}
 ...
 */
export function exportOtsFiles(anchor: AnchorProof): OtsExport[] {
  const out: OtsExport[] = [];
  for (const cal of anchor.calendars) {
    if (!cal.ok || !cal.response) continue;
...
function shortNameFor(url: string): string {
...
export type { AnchorCalendarResponse };
