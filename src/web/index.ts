...
export async function startDashboard(config: ClawConfig, port: number): Promise<void> {
  const server = http.createServer((req, res) => {
    ...
    try {
      ...
      if (url.pathname === "/api/meta") return sendJson(res, { workdir: config.workdir, model: config.model });
      if (url.pathname === "/api/sessions") return sendJson(res, listSessions(config));
