...
async function runExportOts(config: any, args: string[]): Promise<void> {
...
async function runAuditCli(args: string[]): Promise<void> {
  ...
  const labelGreen = (s: string) => chalk.green(s);
  const labelRed = (s: string) => chalk.red(s);
  ...
  const f = (label: string, key: keyof typeof report.checks) => {
  ...
  if (proofFiles.length === 0) {
    ...
    try {
      ...
      if (proofFiles.length > 0) proofFiles = proofFiles.map((f) => path.join("proofs", f));
    } catch {}
  }
  if (proofFiles.length === 0) {
  ...
  if (report.ok) {
    ...
    for (const r of report.reasons) console.log(labelRed(`      · ${r}`));
  }
  process.exit(report.ok ? 0 : 1);
}
 ...
 */
function inspectOtsFile(file: string): { ok: boolean; summary: string } {
  try {
    ...
    if (bytes.length < HEADER.length + 34) return { ok: false, summary: "too short" };
    if (!bytes.subarray(0, HEADER.length).equals(HEADER)) return { ok: false, summary: "bad header magic" };
    const version = bytes[HEADER.length];
    const op = bytes[HEADER.length + 1];
    if (op !== 0x08) return { ok: false, summary: `unexpected file-hash op 0x${op.toString(16)}` };
...
async function runIdentityCli(args: string[]): Promise<void> {
  const config = loadConfig();
