...
async function main() {
  ...
  if (promptArg && promptArg.length > 0) {
    ...
    if (attestor) attestor.record("user_prompt", { content: promptArg });

    agent.pushUser(prepareUserMessage(promptArg, config).content);
    await agent.run((evt) => {
      if (attestor) attestor.onAgentEvent(evt);
    ...
    if (attestor && savedId) {
      try {
        ...
        console.error(
          chalk.dim(
            `[attest] signed ${attestation.header.leafCount} leaf(s), root=${attestation.header.merkleRoot.slice(0, 16)}…, key=${attestation.header.publicKeyId}`
          )
        );
      } catch (e: any) {
