...
export const writeTool: Tool<{ file_path: string; content: string }> = {
  name: "Write",
  description:
    "Write contents to a file. Creates the file if missing, overwrites if it exists. Use absolute paths. For modifying existing files, prefer the Edit tool instead — it only sends the diff.",
