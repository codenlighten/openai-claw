...
export const semanticTool: Tool<{ query: string; k?: number; snippets?: boolean }> = {
  name: "Semantic",
  description:
    "Search the project by semantic meaning, not literal pattern. Returns the top-K most relevant code chunks ranked by embedding similarity. Use this when the user describes what they want by intent (e.g. \"where is auth checked?\") rather than by exact string. Falls back to a helpful error if the index hasn't been built — run /index first.",
  ...
  async run(input, ctx) {
    try {
      ...
      if (hits.length === 0) return ok("(no hits)");
      const lines = hits.map((h, i) => {
        const head = `${i + 1}. ${h.file}#${h.chunkIndex} (score=${h.score.toFixed(3)})`;
        if (!input.snippets) return head;
